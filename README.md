# Mr Niftyfire – Official Website

Welcome to the official website for **Mr Niftyfire** — gamer, creator and proud christian. This site is the main hub for my content, my videos, and fun pages based on the games I play.
This is the website! https://mrniftyfire.github.io/My-Youtube-Website/My%20Youtube%20Website.html
<div align=" center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://mrniftyfire.github.io/Mr-Niftyfire-Website/Mr%20Niftyfire%20Logo.png" />
    <img src="https://mrniftyfire.github.io/Mr-Niftyfire-Website/Mr%20Niftyfire%20Logo.png" width="400" alt="zvec logo" />
  </picture>
</div>

<p align="center">
<center>
  |<a href="https://mrniftyfire.github.io/Mr-Niftyfire-Website/Mr%20Niftyfire%20Website.html">🏠 <strong>Home</strong> </a> |
  <a href="https://www.youtube.com/@MrNiftyfire">🎮 <strong>Youtube</strong> </a>| </center>
</p>

**The Mr Niftyfire Website** is a website where I share information about me, my YouTube channel, my religion and all the different games and topics I make videos about. You’ll find pages for Minecraft, Roblox, Blooket, Duolingo, Geometry Dash, Chess, and more. Each page includes pictures, fun sections, and sometimes hidden secrets.

## 💫 Features & Pages

- A home page introducing me and my content.
- Fun sections with **“Fun Facts!”**
- A YouTube trailer section.
- Buttons linking to different game-themed pages.
- Images and previews of other website sections.
- Hidden secrets or easter eggs if you look carefully.
## 🗂 Website Sections
The website includes (or will include) pages for:
- Minecraft  
- Roblox  
- Blooket  
- Duolingo  
- Geometry Dash (GD)  
- Chess  
- LEGO & Stop Motion  
- Among Us  
- Prodigy  
- Pocket Champs  
- Christianity
- “Uncommon Videos”  
- And more as the site grows

Each page focuses on one topic and includes images, info, challenges, or fun extras.

---

## ✉️ Contact
Contact options.
- [My YouTube Channel](https://www.youtube.com/@MrNiftyfire)
- [My Email](mrniftyfireofficial@outlook.com)

---

## 🔍 Easter Eggs
This website includes secrets! Some pages or images might hide:
- Secret links  
- Hidden text    
- Mystery pages  

Explore and try to find them!

---

## 🛠 Development & Hosting
- Hosted with **GitHub Pages**.  
- Designed using **HTML, CSS, and JavaScript**.  
- Mostly works on both desktop and mobile (mobile layout still being improved).

---

## 📌 Notes & Future Improvements
- Add/Update the YouTube trailer.
- Improve mobile navigation.
- Adding a movie reviews page
- Planning to add a chatbot to help you navigate the website
---

## 🤝 Join Our Community

<div align="center">

Stay updated and get support — scan or click:

<table align="center" style="border-collapse: collapse; margin: 16px auto; width: 100%; max-width: 520px;">
<center>   <tr>
    <td align="center" style="padding: 8px; width: 25%;">
      <div style="font-weight: 600; font-size: 14px; margin-bottom: 6px;">Youtube</div>
      <img src="https://mrniftyfire.github.io/Mr-Niftyfire-Website/Mr%20Niftyfire%20QR%20Code.png" alt="Mr Niftyfire QR Code" width="100" style="border-radius: 8px; border: 1px solid #ddd;">
    </td>
</center>
  </tr>
</table>

</div>

## ❤️ Contributing

We welcome and appreciate contributions from the community! Whether you're fixing a bug or adding a feature, your help makes **The Mr Niftyfire Website** better for everyone.

Check out our [Contributing Guide](./CONTRIBUTING.md) to get started!

Thanks for checking out the **Mr Niftyfire Website**! This site will keep growing as I make more content and add more game pages.



---

## 📘 About This Website
This website is where I share information about me, my YouTube channel, and all the different games and topics I make videos about. You’ll find pages for Minecraft, Roblox, Blooket, Duolingo, Geometry Dash, Chess, and more. Each page includes pictures, fun sections, and sometimes hidden secrets.

---

## 🎮 What You’ll Find
- A home page introducing me and my content.
- Fun sections like **“Fun Bobious Facts!”**
- A YouTube trailer section.
- Buttons linking to different game-themed pages.
- Images and previews of other website sections.
- Hidden secrets or easter eggs if you look carefully.

---

## 🗂 Website Sections
The website includes (or will include) pages for:
- Minecraft  
- Roblox  
- Blooket  
- Duolingo  
- Geometry Dash (GD)  
- Chess  
- LEGO & Stop Motion  
- Among Us  
- Prodigy  
- Pocket Champs  
- “Uncommon Videos”  
- And more as the site grows

Each page focuses on one topic and includes images, info, challenges, or fun extras.

---

## ✉️ Contact
Contact options.
- [My YouTube Channel](https://www.youtube.com/@MrNiftyfire)
- [My Email](mrniftyfireofficial@outlook.com)

---

## 🔍 Easter Eggs
This website includes secrets! Some pages or images might hide:
- Secret links  
- Hidden text    
- Mystery pages  

Explore and try to find them!

---

## 🛠 Development & Hosting
- Hosted with **GitHub Pages**.  
- Designed using **HTML, CSS, and JavaScript**.  
- Mostly works on both desktop and mobile (mobile layout still being improved).

---

## 📌 Notes & Future Improvements
- Add/Update the YouTube trailer.
- Improve mobile navigation.
- Adding a movie reviews page
- Planning to add a chatbot to help you navigate the website
---

Thanks for checking out the **Mr Niftyfire Website**! This site will keep growing as I make more content and add more game pages.

